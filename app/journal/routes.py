...
@login_required
def journal_index():
    entries = JournalEntry.query.filter_by(user_id=current_user.id).order_by(JournalEntry.entry_date.desc()).limit(30).all()
    today = date.today()
    return render_template("journal/index.html", entries=entries, today=today)
